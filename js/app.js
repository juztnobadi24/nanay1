...
function hideAddressBar() {
    ...
    if (navigator.userAgent.match(/iPhone|iPad|iPod/i)) {
        ...
        // Also try to trigger a scroll event
        setTimeout(() => {
            window.dispatchEvent(new Event('scroll'));
        }, 20);
    }
    
    // For Android, sometimes need to scroll twice
...
async function initApp() {
    ...
    setTimeout(() => {
        // No longer calling checkAndApplyFullscreen - it's removed
        // Just hide address bar after layout settles
        hideAddressBar();
    }, 1000);
    
    // Also listen for video play to trigger hide address bar
